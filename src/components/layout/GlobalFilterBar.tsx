...
export default function GlobalFilterBar() {
  ...
  const [market, setMarket] = useState<"SPOT"|"PERP"|"OPTIONS"|"ALL">(filters.market || 'ALL')
  const [from, setFrom] = useState(filters.dateRange?.from || '')
  const [to, setTo] = useState(filters.dateRange?.to || '')

  function apply() {
    setFilters({ symbol: symbol || undefined, market, dateRange: { from: from || undefined, to: to || undefined } })
  }
  ...
  return (
    <div className="w-full bg-zinc-900/40 p-3 rounded-lg flex gap-3 items-center">
      ...
      <select value={market} onChange={(e) => setMarket(e.target.value as "SPOT"|"PERP"|"OPTIONS"|"ALL") } className="select select-sm">
